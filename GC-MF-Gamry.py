...
for file in os.listdir():  # Reads all data files, puts all data in disctionaries
    if file.endswith('.DTA'):
        with open(file, 'r') as f:
            if file.startswith('EIS'):  # Electrochemical Impedance spectroscopy
                ...
                tEIS['{}'.format(file)] = int(
                    int(a[2]) + int(a[1])*60 + int(a[0]) * 3600 + int(int(a2[1]) * 86400))
                for iii in range(len(c)-1):
                    if c[iii][0:6] == 'ZCURVE':
                        s1 = iii + 3
                b = c[s1:]  # EIS data starts at line 54
                t = [[] for _ in range(0, 11)]
                for i in range(len(b)):
                    a1 = b[i].split('\t')
                    for k in range(1, 11):
                        # 0-empty, 1-Pt, 2-Time, 3-Freq, 4-Zreal, 5-Zimag
                        t[k].append(float(a1[k]))
                # 6-Zsig, 7-Zmod, 8-Zphz, 9-Idc, 10-Vdc, 11-IERange
                dEIS['{}'.format(file)] = t
                ...
                for iii in range(len(c)-1):
                    if c[iii][0:5] == 'CURVE':
                        s1 = iii + 3
                b = c[s1:]  # CA data starts at line 61
                t = [[] for _ in range(0, 9)]
                for i in range(len(b)):
                    a1 = b[i].split('\t')
                    for k in range(1, 9):
                        # 0-empty, 1-Pt, 2-Time, 3-Vf, 4-Im, 5-Vu
                        t[k].append(float(a1[k]))
                ...
                for iii in range(len(c)-1):
                    if c[iii][0:5] == 'CURVE':
                        s1 = iii + 3
                b = c[s1:]  # CP data starts at line 64
                t = [[] for _ in range(0, 9)]
                for i in range(len(b)):
                    a1 = b[i].split('\t')
                    for k in range(1, 9):
    ...
    if file.startswith('plotdata'):  # Mass flow
        with open(file, 'r') as f:
            ...
            tMF['{}'.format(file)] = int(int(f.name[21:23]) + int(f.name[19:21])
                                         * 60 + int(f.name[17:19]) * 3600 + int(int(f.name[15:17])) * 86400)
    ...
    if file.endswith('.TX0'):  # Gas chromatography
        with open(file, 'r') as f:
            ...
            a = c[1].split(',')
            a2 = a[3].split('/')
            tGC['{}'.format(file)] = int(int(a[3][-10:-8])*3600 + int(a[3]
                                                                      [-7:-5])*60 + int(a[3][-4:-2]) + int(int(a2[1]) * 86400))
...
if dCV != {}:
    ...
    for i, c in zip(range(len(dCV)), sns.color_palette()):
        ...
        for i in range(len(x1)):
            if x1[i] != 0:
                # "Lines in .DTA that indicate a new CV curve got assigned 0 earlier"
                x.append(x1[i])
                y.append(y1[i])
...
if dCV != {}:
    ...
    for i, c in zip(range(len(dCV)), sns.color_palette()):
        ...
        if (CVstarttime - EISstarttimee) < 600 and CVEISname in list(tEIS.keys()):
            R = list(dEIS[CVEISname])[4][dEIS[CVEISname]
                                         [5].index(max(list(dEIS[CVEISname][5])))]
        ...
        fullCViR_ax.scatter(CVpotential, CVcurrent,
                            color=c, s=0.01, label=str(name))
...
if dCA != {}:
    ...
    for i, c in zip(range(len(dCA)), sns.color_palette()):
        ...
        fullCA_ax.scatter(CAtime[5:], CAcurrent[5:],
                          color=c, s=3, label=str(name))
        partCA_ax.scatter(CAtime[5:], CAcurrent[5:], color=c, s=2)
        partCA_ax.set_ylim(
            [max(CAcurrent[5:])*0.90, min(CAcurrent[5:]) * 1.10])
...
if dCP != {}:
    ...
    for i, c in zip(range(len(dCP)), sns.color_palette()):
        ...
        name = str(name.replace('_', ' '))
        fullCP_ax.scatter(CPtime[5:], CPpotential[5:],
                          color=c, s=3, label=str(name))
        partCP_ax.scatter(CPtime[5:], CPpotential[5:], color=c, s=2)
        partCP_ax.set_ylim(
            [max(CPpotential[5:])*0.90, min(CPpotential[5:]) * 1.10])
    ...
    fullCP_ax.set_ylabel('Potential vs Ag/AgCl [V]')
...
if dMF != {}:
    os.mkdir('Results/GasFlow')
    if dCP != {}:
        for i in dCP:
            x = []
            y = []
            for k in range(len(dCP[i][1])-1):
                if dMF_UTvsF[(dCP[i][1][k]) + tCP[i]] < 1.5 * dMF_UTvsF[(dCP[i][1][k + 1]) + tCP[i]]:
                    x.append(dCP[i][1][k])
                    y.append(dMF_UTvsF[(dCP[i][1][k]) + tCP[i]])
            CPMF_fig, CPMF_ax = plt.subplots()
            name = str(i).replace('_', ' ')
            name = name.replace('.DTA', '')
            CPMF_ax.scatter(x,y, c = 'g', s = 3)
            CPMF_ax.set_xlabel('Time [s]')
            CPMF_ax.set_ylabel('Gas flow [ml/min]')
            CPMF_ax.set_title('Gas flow during ' + str(name))
            CPMF_fig.savefig('Results/GasFlow')
...
if dGCA != {}:
    ...
    # Old value: 0.0003701        #calibration factor Ethylene
    cETH = 0.000362576
    # Old value: 0.00680013       #calibration factor Hydrogen
    cHYD = 0.007054425
    # Old value: 0.00068207       #calibration factor Methanol
    cMET = 0.000715923
    # Old value: 0.00069638       #calibration factor Carbon monoxide
    cCO = 0.000707912
            ...
            if GCAstarttime in dCA_UTvsI and GCAstarttime in dMF_UTvsF:
                ...
                for k in range(len(dGCA[i][1])):
                    if 5.2 <= float(dGCA[i][1][k]) < 5.90:  # Ethylene
                        dGCA[i][2][k] = (
                            ((float(dGCA[i][2][k]) * ((InjectionF * GF1) / 60000)) * F * nETH) / (22.4 * 1000)) * cETH
                        if (dGCA[i][2][k]/1000) >= abs(InjectionI * 0.001):
                            FEeth.append(
                                round(float(abs(dGCA[i][2][k]/(InjectionI * 10))), 2))
                    ...
                    if 9.85 <= float(dGCA[i][1][k]) < 10.40:  # Methanol
                        dGCA[i][2][k] = (
                            ((float(dGCA[i][2][k]) * ((InjectionF * GF1) / 60000)) * F * nMET) / (22.4 * 1000)) * cMET
                        if (dGCA[i][2][k]/1000) >= abs(InjectionI * 0.001):
                            FEmet.append(
                                round(float(abs(dGCA[i][2][k]/(InjectionI * 10))), 2))
                    ...
                    if 10.5 <= float(dGCA[i][1][k]) < 11.4:  # Carbon Monoxide
                        dGCA[i][2][k] = (
                            ((float(dGCA[i][2][k]) * ((InjectionF * GF1) / 60000)) * F * nCO) / (22.4 * 1000)) * cCO
                        if (dGCA[i][2][k]/1000) >= abs(InjectionI * 0.001):
                            FEco.append(
                                round(float(abs(dGCA[i][2][k]/(InjectionI * 10))), 2))
            ...
            if GCBstarttime in dCA_UTvsI and GCBstarttime in dMF_UTvsF:
                ...
                for k in range(len(dGCB[i][1])):
                    if 1.30 <= float(dGCB[i][1][k]) < 1.50:  # Hydrogen
                        dGCB[i][2][k] = (
                            ((float(dGCB[i][2][k]) * ((InjectionF * GF1) / 60000)) * F * nHYD) / (22.4 * 1000)) * cHYD
                        if (dGCB[i][2][k]/1000) >= abs(InjectionI * 0.001):
                            FEhyd.append(
                                round(float(abs(dGCB[i][2][k]/(InjectionI*10))), 2))
    ...
    if tCA != {}:
        for i in range(len(tCA)):
            tCOMB[list(tCA.keys())[i]] = list(tCA.values())[i]

    if tCP != {}:

        for i in range(len(tCP)):
            tCOMB[list(tCP.keys())[i]] = list(tCP.values())[i]
    ...
    Tab_headers = ['Product', 'Potential [V]', 'Current [A]',
                   'After [min]', 'Partial current [mA]', 'FE [%]']
    Tab_values = []
    ...
    for i, c in zip(range(len(Potentials)), sns.color_palette()):
        if FEs[i] != []:
            ...
            fullFE_ax.scatter(Potentials[i], FEs[i],
                              color=c, s=5, label=(Names[i]))
            ...
            for ii in range(len(TIMEs[i])):
                if float(round(float(FEs[i][ii]), 3)) != 0.000:
                ...
                Tab_values.append([t1, Potentials[i][ii], Currents[i][ii], TIMEs[i][ii], round(
                    abs(Currents[i][ii] * 10 * FEs[i][ii]), 1), FEs[i][ii]])
...
if dGCA != {}:
    ...
    title = workbook.add_format(
        {'bold': True, 'font_size': 14, 'bg_color': '#DBDBDB'})
    bold = workbook.add_format(
        {'bold': True, 'font_size': 12, 'bg_color': '#D1FFD9'})
